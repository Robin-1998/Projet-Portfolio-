...
-- -----------------------------
-- ==================== RÉGIONS ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('Gondor', 'region', 'Royaume des Hommes du Sud, gardien contre Mordor', NULL, '/place/Gondor.png'),
('Rohan', 'region', 'Royaume des Cavaliers du Rohan', NULL, '/place/rohan.png'),
('Mordor', 'region', 'Terre sombre de Sauron', NULL, '/place/Mordor.png'),
('Eriador', 'region', 'Région de l''Ouest, incluant la Comté', NULL, '/place/Eriador.png'),
('Rhovanion', 'region', 'Terres Sauvages de l''Est', NULL, '/place/Rhovanion.png')
ON CONFLICT (title) DO NOTHING;

-- ==================== LIEUX - GONDOR ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('Minas Tirith', 'capitale', 'Capitale du Gondor, la Cité Blanche', (SELECT id FROM places WHERE title='Gondor'), '/place/minas-tirith.png'),
('Osgiliath', 'ruine', 'Ancienne capitale en ruines sur l''Anduin', (SELECT id FROM places WHERE title='Gondor'), '/place/osgiliath.png'),
('Champ de Pelennor', 'plaine', 'Grande plaine devant Minas Tirith, site de bataille', (SELECT id FROM places WHERE title='Gondor'), '/place/champ_de_pelennor.png'),
('Minas Morgul', 'forteresse', 'Cité maudite, autrefois Minas Ithil', (SELECT id FROM places WHERE title='Gondor'), 'place/minas_morgul.png')
ON CONFLICT (title) DO NOTHING;

-- ==================== LIEUX - ROHAN ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('Edoras', 'capitale', 'Capitale du Rohan, siège du Roi Théoden', (SELECT id FROM places WHERE title='Rohan'), '/place/Edoras.png'),
('Isengard', 'forteresse', 'Forteresse de Saroumane avec la tour d''Orthanc', (SELECT id FROM places WHERE title='Rohan'), '/place/Isengard.png'),
('Fangorn', 'foret', 'Ancienne forêt des Ents', (SELECT id FROM places WHERE title='Rohan'), '/place/Fangorn.png'),
('Gouffre de Helm', 'forteresse', 'Forteresse imprenable dans les montagnes', (SELECT id FROM places WHERE title='Rohan'), '/place/Gouffre_de_helm.png'),
('Passage des Morts', 'chemin', 'Chemin hanté menant à travers les montagnes', (SELECT id FROM places WHERE title='Rohan'), '/place/Passage_des_Morts.png')
ON CONFLICT (title) DO NOTHING;

-- ==================== LIEUX - MORDOR ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('Montagne du Destin (Orodruin)', 'montagne', 'Volcan où l''Anneau Unique fut forgé', (SELECT id FROM places WHERE title='Mordor'), '/place/montagne_du_destin.png'),
('Barad-dûr', 'dark', 'Tour Sombre, forteresse de Sauron', (SELECT id FROM places WHERE title='Mordor'), '/place/Barad-dûr.png'),
('Cirith Ungol', 'forteresse', 'Passage gardé par l''araignée Arachne', (SELECT id FROM places WHERE title='Mordor'), '/place/Cirith_Ungol.png')
ON CONFLICT (title) DO NOTHING;

-- ==================== LIEUX - ERIADOR ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('La Comté', 'capitale', 'Terre paisible des Hobbits', (SELECT id FROM places WHERE title='Eriador'), '/place/comte.png'),
('Hobbitebourg', 'ville', 'Principal village de la Comté', (SELECT id FROM places WHERE title='Eriador'), '/place/Hobbitebourg.png'),
('Cul-de-Sac', 'ville', 'Demeure de Bilbon et Frodon Sacquet', (SELECT id FROM places WHERE title='Eriador'), '/place/Cul_de_sac.png'),
('Bree', 'ville', 'Village où Hommes et Hobbits cohabitent', (SELECT id FROM places WHERE title='Eriador'), '/place/Bree.png'),
('Fondcombe', 'ville', 'Rivendell, refuge elfique d''Elrond', (SELECT id FROM places WHERE title='Eriador'), '/place/Fondcombe.png'),
('Havre Gris', 'port', 'Port elfique vers les Terres Immortelles', (SELECT id FROM places WHERE title='Eriador'), '/place/Havre_gris.png'),
('Amon Sûl', 'ruine', 'Ruines de la Tour de Garde (Mont Venteux)', (SELECT id FROM places WHERE title='Eriador'), '/place/Amon_Sûl.png'),
('Fourré aux Trolls', 'foret', 'Lieu où Bilbon rencontra les trolls', (SELECT id FROM places WHERE title='Eriador'), '/place/Fourré_aux_Trolls.png'),
('Moria', 'mine', 'Ancienne cité naine de Khazad-dûm', (SELECT id FROM places WHERE title='Eriador'), '/place/moria.png'),
('Col de Caradhras', 'montagne', 'Col périlleux dans les Monts Brumeux', (SELECT id FROM places WHERE title='Eriador'), '/place/Col_de_Caradhras.png')
ON CONFLICT (title) DO NOTHING;

-- ==================== LIEUX - RHOVANION ====================
INSERT INTO places (title, type_place, description, parent_id, image_url)
VALUES
('Erebor (Montagne Solitaire)', 'forteresse', 'Royaume sous la Montagne, reconquis par Thorin', (SELECT id FROM places WHERE title='Rhovanion'), '/place/erebor.png'),
('Forêt Noire', 'foret', 'Forêt sombre et dangereuse, ancien Vertbois le Grand', (SELECT id FROM places WHERE title='Rhovanion'), 'place/foret_noire.png'),
('Lothlórien', 'foret', 'Royaume elfique de Galadriel et Celeborn', (SELECT id FROM places WHERE title='Rhovanion'), '/place/Lothlórien.png'),
('Caras Galadhon', 'ville', 'Cité des arbres au cœur de la Lothlórien', (SELECT id FROM places WHERE title='Rhovanion'), '/place/Caras_Galadhon.png'),
('Champs aux Iris', 'plaine', 'Plaine où périt Isildur', (SELECT id FROM places WHERE title='Rhovanion'), '/place/Champs_aux_Iris.png'),
('Emyn Muil', 'montagne', 'Collines rocheuses et labyrinthiques', (SELECT id FROM places WHERE title='Rhovanion'), '/place/Emyn_Muil.png'),
('Statues des Rois d''Argonath', 'monument', 'Piliers des Rois, gardiens de l''Anduin', (SELECT id FROM places WHERE title='Rhovanion'), '/place/statue.png'),
('Marais des Morts', 'eau', 'Marécages hantés par les morts de batailles anciennes', (SELECT id FROM places WHERE title='Rhovanion'), '/place/marais_des_morts.png')
ON CONFLICT (title) DO NOTHING;
...
('Boromir', 2978, 3019, 'Troisième âge', 'Troisième âge', 'Masculin', 'Guerrier et membre de la communauté de l''anneau', $$ Boromir est un vaillant guerrier du Gondor, fils du régent Denethor. Homme courageux et dévoué à son peuple, il cherche constamment à protéger sa cité de Minas Tirith. Malgré ses qualités de combattant exceptionnel et son sens de l''honneur, il est rongé par le désir de sauver son royaume, ce qui le rend vulnérable à la tentation de l''Anneau. Sa mort héroïque en défendant Merry et Pippin rachète ses erreurs et révèle la noblesse de son cœur. $$, 'Le Gondor a besoin de force pour se défendre.', (SELECT id FROM races WHERE name='Humain')),
('Gollum', 2440, 3019, 'Troisième âge', 'Troisième âge', 'Masculin', 'Ancien porteur de l''Anneau', $$ Gollum, autrefois nommé Sméagol, était un Hobbit avant d''être corrompu par l''Anneau Unique qu''il a possédé pendant près de 500 ans. Cette longue possession a déformé son corps et son esprit, le transformant en une créature maigre et torturée, partagée entre sa personnalité originelle de Sméagol et celle, maléfique, de Gollum. Obsédé par son "précieux", il vit dans l''ombre, se nourrissant de poissons crus et fuyant la lumière. Malgré sa nature corrompue, il reste une créature pitoyable, victime tragique du pouvoir de l''Anneau. $$, 'Mon précieux... il nous l''a volé, le méchant Hobbit. Nous le voulons, nous le reprendrons !', NULL),
('Legolas', NULL, NULL, 'Troisième âge', NULL, 'Masculin', 'Guerrier et membre de la communauté de l''anneau', $$ Legolas est le fils du roi Thranduil de la Forêt Noire, un elfe sylvain d''une grâce et d''une agilité exceptionnelles. Archer d''élite et membre de la Communauté de l''Anneau, il possède une vue perçante et des réflexes surhumains qui font de lui un combattant redoutable. Immortel et léger comme le vent, il peut marcher sur la neige sans laisser de traces. Son amitié improbable avec le nain Gimli transcende les anciennes rivalités entre leurs peuples. Après la chute de Sauron, il parcourt la Terre du Milieu avant de naviguer vers les Terres Immortelles. $$, 'Mes yeux perçoivent ce que les vôtres ne peuvent voir', (SELECT id FROM races WHERE name='Elfe')),
('Bilbon Sacquet', 2890, NULL, 'Troisième âge', NULL, 'Masculin', 'Ancien porteur de l''Anneau', $$ Bilbon Sacquet est un Hobbit de la Comté qui a mené une vie tranquille avant d''être entraîné dans une aventure extraordinaire avec Thorin Écu-de-Chêne et sa compagnie de nains. C''est lors de cette quête qu''il découvre l''Anneau Unique dans les tunnels des Monts Brumeux, après sa rencontre avec Gollum. Cet anneau prolonge anormalement sa vie et influence son comportement. Érudit et auteur, il rédige ses mémoires dans son livre "Là et Retour". Oncle et mentor de Frodon, il lui lègue l''Anneau avant de partir pour Fondcombe. Finalement, il quitte la Terre du Milieu pour les Terres Immortelles, l''un des rares mortels à recevoir cet honneur. $$, 'J’aime moins la moitié d’entre vous à moitié aussi bien que vous le méritez.', NULL),
('Frodon Sacquet', 2968, NULL, 'Troisième âge', NULL, 'Masculin', 'Porteur de l''anneau et membre de la communauté de l''anneau', $$ Frodon Sacquet est un Hobbit de la Comté, neveu et héritier de Bilbon. Jeune et curieux, il hérite de l''Anneau Unique et accepte la lourde responsabilité de le détruire en le jetant dans les feux du Mont Destin. Malgré sa petite taille et son apparence fragile, il fait preuve d''un courage et d''une détermination extraordinaires face aux épreuves. Le poids de l''Anneau le marque profondément, physiquement et mentalement. Après avoir sauvé la Terre du Milieu, les blessures qu''il porte ne guérissent jamais complètement. Il quitte finalement la Comté en 3021 pour rejoindre les Terres Immortelles avec Bilbon et Gandalf, où il pourra enfin trouver la paix. $$, 'Je porterai ce fardeau, même si je ne connais pas le chemin. Quelqu''un doit le faire' , NULL),
('Azog', NULL, 2799, 'Troisième âge', 'Troisième âge', 'Masculin', 'Guerrier et chef des armées', $$ Azog le Profanateur est un redoutable chef orque de la Moria, connu pour sa cruauté et sa férocité au combat. D''une taille imposante pour un orque, il se distingue par sa peau pâle et les nombreuses cicatrices qui marquent son corps. Il est responsable de la mort du roi nain Thrór, qu''il décapite et profane, déclenchant ainsi la guerre entre les Nains et les Orques. Durant la bataille d''Azanulbizar, il affronte Thorin Écu-de-Chêne et lui inflige une profonde blessure, mais perd son bras gauche dans le combat. Animé par une haine féroce envers la lignée de Durin, il poursuit Thorin pendant des décennies avant de finalement périr lors de la Bataille des Cinq Armées. $$, 'Aucun nain n''échappera à ma vengeance', (SELECT id FROM races WHERE name='Orc')),
('Aragorn', 2931, 120, 'Troisième âge', 'Quatrième âge', 'Masculin', 'Guerrier, rôdeur, membre de la communauté de l''anneau, roi et chef des armées', $$ Aragorn, fils d''Arathorn, est l''héritier légitime du trône du Gondor et descendant direct d''Isildur. Élevé à Fondcombe sous le nom d''Estel, il vit pendant des années comme rôdeur du Nord, protégeant secrètement la Comté et les terres libres. Guerrier accompli et tacticien hors pair, il manie l''épée reforgée Andúril, héritée de ses ancêtres. Membre de la Communauté de l''Anneau, il guide et protège les Hobbits dans leur périple. Son amour pour l''elfe Arwen le pousse à reconquérir son trône. Après la défaite de Sauron, il devient le roi Elessar, unifiant les royaumes du Gondor et de l''Arnor, inaugurant une ère de paix et de prospérité qui durera tout son règne. $$, 'Un jour viendra où le courage des hommes faiblira, mais ce jour n''est pas encore venu.' , (SELECT id FROM races WHERE name='Humain')),
('Grima', NULL, 3019, 'Troisième âge', 'Troisième âge', 'Masculin', 'Espion', $$ Grima, surnommé Langue de Serpent, est le conseiller du roi Théoden du Rohan. Homme sournois et manipulateur, il est en réalité un agent de Saroumane, empoisonnant l''esprit du roi par ses paroles trompeuses et affaiblissant le royaume de l''intérieur. Son apparence chétive et son regard fuyant trahissent sa nature lâche et traîtresse. Secrètement amoureux d''Éowyn, nièce du roi, il espère l''obtenir comme récompense de sa trahison. Après avoir été démasqué par Gandalf, il fuit vers l''Isengard où il devient le serviteur misérable de Saroumane. Sa fin tragique survient lorsqu''il assassine son maître dans un accès de rage et de désespoir, avant d''être lui-même abattu par les archers hobbits. $$, 'Pourquoi risquer la guerre quand nous pouvons négocier la paix ? Écoutez mes conseils...', (SELECT id FROM races WHERE name='Humain'))
ON CONFLICT (name) DO NOTHING;
